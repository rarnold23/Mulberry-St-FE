...
function initializeElements() {
    ...
    const showSongListMobileBtn = document.getElementById('show-song-list-mobile');
...
function setupEventListeners() {
    ...
    const showSongListMobileBtn = document.getElementById('show-song-list-mobile');
    if (showSongListMobileBtn) {
        showSongListMobileBtn.addEventListener('click', showSongList);
    }
...
function showSongList() {
    ...
    setTimeout(() => {
        ...
        console.log('  - Content container before fade-in:');
        console.log('    - Classes:', contentContainer.className);
        console.log('    - Computed opacity:', window.getComputedStyle(contentContainer).opacity);
        console.log('    - Computed transform:', window.getComputedStyle(contentContainer).transform);
        console.log('    - Computed transition:', window.getComputedStyle(contentContainer).transition);
        
        contentContainer.classList.add('fade-in');
        console.log('  - After adding fade-in class:', contentContainer.className);
        
        // Check computed styles after adding class
        setTimeout(() => {
            console.log('  - Content container 10ms after fade-in:');
            console.log('    - Classes:', contentContainer.className);
            console.log('    - Computed opacity:', window.getComputedStyle(contentContainer).opacity);
            console.log('    - Computed transform:', window.getComputedStyle(contentContainer).transform);
            console.log('    - Computed transition:', window.getComputedStyle(contentContainer).transition);
        ...
        // Check again after transition should have started
        setTimeout(() => {
            console.log('  - Content container 100ms after fade-in:');
            console.log('    - Classes:', contentContainer.className);
            console.log('    - Computed opacity:', window.getComputedStyle(contentContainer).opacity);
            console.log('    - Computed transform:', window.getComputedStyle(contentContainer).transform);
        }, 100);
        
        // Check at the end of the fade-in transition
        setTimeout(() => {
            console.log('  - Content container 600ms after fade-in (end of transition):');
            console.log('    - Classes:', contentContainer.className);
            console.log('    - Computed opacity:', window.getComputedStyle(contentContainer).opacity);
            console.log('    - Computed transform:', window.getComputedStyle(contentContainer).transform);
        }, 600);
...
function showPlayer() {
    ...
    // 1. Start content fade-out animation first
    console.log('🔧 Step 1: Triggering content fade-out');
    console.log('  - Content container before fade-out:');
    console.log('    - Classes:', contentContainer.className);
    console.log('    - Computed opacity:', window.getComputedStyle(contentContainer).opacity);
    console.log('    - Computed transform:', window.getComputedStyle(contentContainer).transform);
    console.log('    - Computed transition:', window.getComputedStyle(contentContainer).transition);
    ...
    console.log('  - After removing fade-in class:', contentContainer.className);
    
    contentContainer.classList.add('fade-out');
    console.log('  - After adding fade-out class:', contentContainer.className);
    
    // Check computed styles after adding fade-out class
    setTimeout(() => {
        console.log('  - Content container 10ms after fade-out:');
        console.log('    - Classes:', contentContainer.className);
        console.log('    - Computed opacity:', window.getComputedStyle(contentContainer).opacity);
        console.log('    - Computed transform:', window.getComputedStyle(contentContainer).transform);
        console.log('    - Computed transition:', window.getComputedStyle(contentContainer).transition);
    ...
    // Check again after transition should have started
    setTimeout(() => {
        console.log('  - Content container 100ms after fade-out:');
        console.log('    - Classes:', contentContainer.className);
        console.log('    - Computed opacity:', window.getComputedStyle(contentContainer).opacity);
        console.log('    - Computed transform:', window.getComputedStyle(contentContainer).transform);
    }, 100);
    
    // Check at the end of the fade-out transition
    setTimeout(() => {
        console.log('  - Content container 450ms after fade-out (end of transition):');
        console.log('    - Classes:', contentContainer.className);
        console.log('    - Computed opacity:', window.getComputedStyle(contentContainer).opacity);
        console.log('    - Computed transform:', window.getComputedStyle(contentContainer).transform);
    }, 450);
    
    // 2. Start SVG background slide-down animation (after content starts fading)
    setTimeout(() => {
    ...
    // 3. Show player view after fade-out animation completes
    setTimeout(() => {
    ...
    setTimeout(() => {
        console.log('🔧 Step 4: Hiding song list view (after animations complete)');
    ...
    }, 800); // Wait for fade-out (450ms) + slide-down (600ms) to complete with extra buffer
    ...
    setTimeout(() => {
        ...
        console.log('  - SVG background computed opacity:', window.getComputedStyle(svgBackground).opacity);
        console.log('  - SVG background computed bottom:', window.getComputedStyle(svgBackground).bottom);
    }, 1000); // Increased to ensure animations complete with extra buffer
}
...
// Debug function to check content container state
function debugContentContainer() {
    console.log('🔍 DEBUG: Content Container State');
    console.log('  - Element:', contentContainer);
    console.log('  - Classes:', contentContainer ? contentContainer.className : 'N/A');
    console.log('  - Computed opacity:', contentContainer ? window.getComputedStyle(contentContainer).opacity : 'N/A');
    console.log('  - Computed transform:', contentContainer ? window.getComputedStyle(contentContainer).transform : 'N/A');
    console.log('  - Computed transition:', contentContainer ? window.getComputedStyle(contentContainer).transition : 'N/A');
    console.log('  - Computed visibility:', contentContainer ? window.getComputedStyle(contentContainer).visibility : 'N/A');
    console.log('  - Computed display:', contentContainer ? window.getComputedStyle(contentContainer).display : 'N/A');
    console.log('  - Parent display:', contentContainer && contentContainer.parentElement ? window.getComputedStyle(contentContainer.parentElement).display : 'N/A');
    console.log('  - Current view:', currentView);
    console.log('  - Is transitioning:', isTransitioning);
}

// Make debug function available globally
window.debugContentContainer = debugContentContainer;
